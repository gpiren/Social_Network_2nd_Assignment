...
# Örnek veri: 4 özellik + ikili sınıf
X <- metrics %>% dplyr::select(common_neighbors, jaccard, adamic_adar, pref_attach)
y <- factor(metrics$class)
...
# Neural network modeli (tek gizli katman, 5 nöron)
nn_model <- nnet(X_train, class.ind(y_train), size = 5, maxit = 500, decay = 0.01, softmax = TRUE)

# Tahmin
pred_probs <- predict(nn_model, X_test, type = "raw")
pred_labels <- factor(ifelse(pred_probs[, 2] > 0.5, "1", "0"))

# Confusion matrix
confusionMatrix(pred_labels, y_test, positive = "1")
...
```{r}
library(xgboost)
X <- as.matrix(metrics[, 1:4])
y <- as.numeric(metrics$class)
...
model_xgb <- xgboost(data = X_train, label = y_train, nrounds = 100, objective = "binary:logistic")
pred_probs <- predict(model_xgb, X_test)
pred_labels <- ifelse(pred_probs > 0.5, 1, 0)

confusionMatrix(as.factor(pred_labels), as.factor(y_test), positive = "1")

```

```{r}
X_test_df <- as.data.frame(X_test)  # Matriksten emin olalım
X_test_df$class <- as.factor(y_test)

library(ggplot2)

ggplot(X_test_df, aes(x = common_neighbors, fill = class)) +
  geom_density(alpha = 0.5) +
  labs(title = "Distribution of Common Neighbors by Class")
